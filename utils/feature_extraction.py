...
def initiate_visual_module(feature_extractor, pretrained_mvit=True):
    ...
    if feature_extractor == 'resnet':
        ...
        # visual_model, _, _ = open_clip.create_model_and_transforms("ViT-B-32", pretrained="openai")
...
def initiate_text_module(feature_extractor):
    ...
    if feature_extractor == 'bert':
        ...
        # text_model, _, _ = open_clip.create_model_and_transforms("ViT-B-32", pretrained="openai")
        tokenizer = get_tokenizer("basic_english")
    elif feature_extractor == 'coca':
        embed_size = 512
...
def extract_text_features(hypotheses, model, feature_extractor, tokenizer):
    with torch.no_grad():
        ...
        if feature_extractor == 'bert':
            ...
            with torch.no_grad():
                tokenizer_out = tokenizer(hypotheses)
                tokenizer_out = tokenizer_out[:,:-1].cuda()
                # tokenizer_out = torch.ones(3,76,dtype=torch.int64).cuda()
