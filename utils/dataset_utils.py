...
def sample_vid_ctv(filename, sample_rate=1, type='rgb'):
    video_frames = []
    video = cv2.VideoCapture(filename) # "/mnt/disk2/huangzhiyi/CrossTask_Verification/test/--G-5UoUQ5A.mp4"
    # print(video.get(cv2.CAP_PROP_FPS))
    success = video.grab()
    fno = 0
    while success:
...
def check_alignment_feats(pred_alignment: List[List[Tuple]], segment_labels: List[List], ent_labels):
    """
    checks if the predicted (dynamic programming-based) alignment is correct
    for positively entailed hypotheses
    """
